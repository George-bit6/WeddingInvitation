...
export default function App() {
  ...
  const getInvitants = async () => {
    ...
    setErrorData(false);
    try {
      const { data, error } = await supabase.rpc("getinvitants");
      if (error) {
        setErrorData(true);
      } else {
        setTableData(data || []);
  ...
  const addInvitant = async (full_name, number_guests, phone_number) => {
    try {
      ...
      const { error } = await supabase.rpc("addinvitant", {
        ...
        p_phone_number: cleanPhone, // 👈 Now sending a numeric typea
      });
  ...
  return (
    <VStack gap={"48px"}>
      ...
      <SimpleGrid columns={3} gap={"72px"}>
        <GridItem colSpan={3}>
          <VStack gap={"48px"}>
            <Heading {...font2} fontSize={"3xl"} textAlign={"center"}>
              Invitants
            </Heading>
            <Heading {...font2} fontSize={"5xl"}>
              {pendingNumber + acceptedNumber + rejectedNumber}
            </Heading>
          </VStack>
        </GridItem>
        <GridItem>
          <VStack>
            <Heading {...font2} fontSize={"xl"} textAlign={"center"}>
              Pending
            </Heading>
      ...
      <VStack>
        ...
        <HStack>
          <Button {...font2} onClick={() => getInvitants()}>
            Refresh Table
          </Button>
          ...
          <Table.Root showColumnBorder size={"sm"} variant={"outline"}>
            <Table.Header>
              <Table.Row>
                ...
                <Table.ColumnHeader {...font2}>Max Capacity</Table.ColumnHeader>
                {/* NEW COLUMN */}
                <Table.ColumnHeader {...font2} color="cyan.400">
                  Guests Coming
                </Table.ColumnHeader>
            ...
            <Table.Body>
              {tableData.map((node, i) => (
                <Table.Row key={node.id}>
                  ...
                  {/* NEW DATA CELL */}
                  <Table.Cell {...font2} fontWeight="bold" color="cyan.300">
                    {node.nb_guests_chosen}
                  </Table.Cell>
